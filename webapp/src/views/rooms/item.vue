...
.c-room(v-if="room", :class="{'standalone-chat': modules['chat.native'] && room.modules.length === 1}")
	...
	chat(v-if="room.modules.length === 1 && modules['chat.native']", :key="room.id", :room="room", :module="modules['chat.native']", mode="standalone")
	.room-sidebar(v-else-if="modules['chat.native'] || modules['question'] || modules['poll']", :class="unreadTabsClasses", role="complementary")
		bunt-tabs(v-if="(!!modules['question'] + !!modules['poll'] + !!modules['chat.native']) > 1 && activeSidebarTab", :activeTab="activeSidebarTab")
		...
		chat(v-if="modules['chat.native']", v-show="activeSidebarTab === 'chat'", :key="room.id", :room="room", :module="modules['chat.native']", mode="compact", @change="changedTabContent('chat')")
