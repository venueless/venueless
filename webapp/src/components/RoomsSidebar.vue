		...
		scrollbars(y)
			...
			.chats
				...
				router-link.text-chat(v-for="chat of roomsByType.textChat", :to="chat === rooms[0] ? {name: 'home'} : {name: 'room', params: {roomId: chat.id}}", :class="{unread: hasUnreadMessages(chat.modules[0].channel_id)}")
					.name {{ chat.name }}
			template(v-if="hasPermission('world:users.list') || hasPermission('world:update')")
				.buffer
				.group-title
				...
				.admin
					...
					router-link.room(:to="{name: 'admin:rooms'}", v-if="hasPermission('world:update')") {{ $t('RoomsSidebar:admin-rooms:label') }}
