...
transition(name="sidebar")
	.c-rooms-sidebar(v-show="show && !snapBack", :style="style", role="navigation", @pointerdown="onPointerdown", @pointermove="onPointermove", @pointerup="onPointerup", @pointercancel="onPointercancel")
		router-link.logo(v-if="$mq.above['m']", to="/", :class="{'fit-to-width': theme.logo.fitToWidth}")
		...
		scrollbars(y)
			...
			.group-title#chats-title(v-if="roomsByType.videoChat.length || roomsByType.textChat.length || hasPermission('world:rooms.create.chat') || hasPermission('world:rooms.create.bbb')")
				...
				.buffer
				bunt-icon-button(v-if="hasPermission('world:rooms.create.chat') || hasPermission('world:rooms.create.bbb')", tooltip="Create Channel", :tooltipFixed="true", @click="showChatCreationPrompt = true") plus
				bunt-icon-button(v-if="worldHasTextChannels", tooltip="Browse all channels", :tooltipFixed="true", @click="showChannelBrowser = true") compass-outline
			.chats(role="group", aria-describedby="chats-title")
				router-link.video-chat(v-for="chat of roomsByType.videoChat", :to="chat === rooms[0] ? {name: 'home'} : {name: 'room', params: {roomId: chat.id}}", :class="{active: chat.id === $route.params.roomId, 'starts-with-emoji': startsWithEmoji(chat.name)}")
					...
					i.bunt-icon.activity-icon.mdi(v-if="chat.users === 'many' || chat.users === 'few'", v-tooltip.bottom.fixed="{text: $t('RoomsSidebar:users-tooltip:' + chat.users)}", :class="{'mdi-account-group': (chat.users === 'many'), 'mdi-account-multiple': (chat.users === 'few')}", :aria-label="$t('RoomsSidebar:users-tooltip:' + chat.users)")
			...
			.group-title#dm-title(v-if="directMessageChannels.length || hasPermission('world:chat.direct')")
				...
				bunt-icon-button(v-if="hasPermission('world:chat.direct')", tooltip="open a direct message", :tooltipFixed="true", @click="showDMCreationPrompt = true") plus
			...
			template(v-if="hasPermission('world:users.list') || hasPermission('world:update') || hasPermission('world:announce') || hasPermission('room:update')")
				...
				.admin
					router-link.room(v-if="hasPermission('world:announce')", :to="{name: 'admin:announcements'}") {{ $t('RoomsSidebar:admin-announcements:label') }}
					router-link.room(v-if="hasPermission('world:users.list')", :to="{name: 'admin:users'}") {{ $t('RoomsSidebar:admin-users:label') }}
					router-link.room(v-if="hasPermission('room:update')", :to="{name: 'admin:rooms:index'}") {{ $t('RoomsSidebar:admin-rooms:label') }}
					router-link.room(v-if="hasPermission('world:users.manage')", :to="{name: 'admin:kiosks:index'}") {{ $t('RoomsSidebar:admin-kiosks:label') }}
	...
	computed: {
		...
		roomsByType () {
			...
			for (const room of this.rooms) {
				...
				} else if (room.modules.some(module => ['call.bigbluebutton', 'call.janus', 'call.digitalsamba', 'call.zoom'].includes(module.type))) {
